function [EIT] = openmeeg_EIT(elec, vol)
...
try
    ...
    if ~ispc
      ...
      fprintf(efid,['om_assemble' str ' ./',geomfile,' ./',condfile,' ./',electrodefile,' ./',eitfile,' 2>&1 > /dev/null\n']);
    else
      fprintf(efid,['om_assemble' str ' ./',geomfile,' ./',condfile,' ./',electrodefile,' ./',eitfile,'\n']);
    end
...
try
    ...
    disp(['Assembling OpenMEEG EIT matrix']);
    stopwatch = tic;
    if ispc
        dos([exefile]);
    else
    ...
    EIT = om_load_full(eitfile,'binary');
    toc(stopwatch);
    cleaner(vol,bndfile,condfile,geomfile,exefile,electrodefile,eitfile)
    cd(tmpfolder)
catch
