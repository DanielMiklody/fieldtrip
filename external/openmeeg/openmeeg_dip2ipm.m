function [gain] = openmeeg_dip2ipm(vol,pnts,dippos)
...
try
    ...
    % handle dipole file
    ndip = size(dippos,1);
    dippos = [kron(dippos,ones(3,1)),kron(ones(ndip,1),eye(3))]; % save pos with each 3D orientation
    om_save_full(dippos,dipfile,'ascii');
    ...
    if ~ispc
      ...
      fprintf(efid,['om_assemble' str ' ./',geomfile,' ./',condfile,' ./',dipfile,' ./',positionfile,' ./',h2ipmfile,' 2>&1 > /dev/null\n']);
    else
      fprintf(efid,['om_assemble' str ' ./',geomfile,' ./',condfile,' ./',dipfile,' ./',positionfile,' ./',h2ipmfile,'\n']);
    end
...
try
    ...
    disp(['Assembling OpenMEEG DS2IPM matrix']);
    stopwatch = tic;
    if ispc
        dos([exefile]);
    else
