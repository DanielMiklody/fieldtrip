function [msm] = openmeeg_msm(pos, vol, flag)

% OPENMEEG_DSM computes the OpenMEEG MSM matrix
%              i.e. Right hand side in the potential equation
%
% Use as
%   [msm] = openmeeg_msm(po, vol, flag)
%
...
try
    ...
    disp('Assembling OpenMEEG MSM matrix');
    stopwatch = tic;
    if ispc
        dos(exefile);
    else
    ...
    msm = om_load_full(dsmfile,'binary');
    toc(stopwatch);
