...
elseif ismeg
    switch ft_headmodeltype(headmodel)
        ...
        case 'openmeeg'
            ...
            %       [h2sens,ds2sens] = ft_sensinterp_openmeeg(dippos, headmodel, sens);
            %       if isempty(dsm)
            %         msm            = ft_sysmat_openmeeg(dippos, headmodel, sens, nonadaptive);
            %         msm1            = ft_sysmat_openmeeg(dippos, headmodel, sens, nonadaptive);
            %         msm2            = ft_sysmat_openmeeg(dippos, headmodel, sens, nonadaptive);
            %         msm3            = ft_sysmat_openmeeg(dippos, headmodel, sens, nonadaptive);
            %       end
            %       lf               = ds2sens + h2sens*headmodel.mat*dsm;
      if isfield(sens, 'tra')
        % compute the leadfield for each gradiometer (linear combination of coils)
...
elseif iseeg
    switch ft_headmodeltype(headmodel)
        ...
        case {'openmeegTripole','openmeegTripole2','openmeegTripole3'}
            ft_hastoolbox('openmeeg', 1);
            nonadaptive = ft_getopt(varargin, 'nonadaptive');
